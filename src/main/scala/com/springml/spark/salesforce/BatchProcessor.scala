...
object BatchProcessor {
  ...
  case class BatchResult(fileName: String)

  @annotation.tailrec
  def retry[T](n: Int, pause: Duration)(fn: => T): T = {
   ...
   */
  @tailrec def awaitSuccess[BatchResult](
    ...
    Await.ready(first, Duration.Inf).value match {
      ...
      case Some(Success(_)) =>
        if (!batchResult.isDefined) {
          ...
          batchResultPromise completeWith first.asInstanceOf[scala.concurrent.Future[BatchProcessor.BatchResult]]
        }
  ...
  def sample = {
    //    promiseFuture onSuccess {
    //      case result => { println("HEREEEEEEEEEEEEE"); batchResult = Some(result); batchResult; }
